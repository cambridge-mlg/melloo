...
def plot_decision_regions(self, centroids, points, labels, out_name, plot_conf, model, device):
    ...
    grid_logits = F.softmax(model.classify(points, grid_tensor).cpu(), dim=1)
    ...
    for c in range(num_classes):
        mask_c = (labels == c)
        plt.scatter(*zip(*(points[mask_c])), marker='.', c=plot_conf.colors[c],
                    alpha=0.8, edgecolors=plot_conf.edge_colors[c], s=150)
        
        plt.scatter(*zip(*(centroids[c])), marker='o', c=plot_conf.colors[c], edgecolors=plot_conf.edge_colors[c], s=250, alpha=0.8)
